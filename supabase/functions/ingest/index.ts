...
const VALID_SOURCE_TYPES = ["youtube", "bandcamp", "soundcloud", "manual"] as const;
...
function validatePayload(body: unknown): ValidationError[] {
  ...
  if (!payload.raw_title || typeof payload.raw_title !== "string" || payload.raw_title.trim() === "") {
    errors.push({ field: "raw_title", message: "raw_title es obligatorio y no puede estar vacío" });
  }

  // source_type debe pertenecer al enum
  if (!payload.source_type || !VALID_SOURCE_TYPES.includes(payload.source_type as SourceType)) {
    errors.push({
      field: "source_type",
  ...
  if (payload.note !== undefined && payload.note !== null) {
    if (typeof payload.note !== "string") {
      ...
      errors.push({ field: "note", message: "note no puede superar 140 caracteres" });
    }
  }

  // url debe ser string si está presente (puede ser null explícito)
  if (payload.url !== undefined && payload.url !== null && typeof payload.url !== "string") {
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  if (!expectedSecret) {
    console.error("GMC_INGEST_SECRET no está configurado en los secrets de la Edge Function");
    return new Response(JSON.stringify({ error: "Configuración interna incorrecta" }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  ...
  if (errors.length > 0) {
    return new Response(JSON.stringify({ error: "Payload inválido", details: errors }), {
      status: 400,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  ...
  if (payload.url) {
    ...
    if (lookupError) {
      ...
      return new Response(JSON.stringify({ error: "Error interno al verificar duplicados" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (existing) {
      return new Response(JSON.stringify({ error: "URL ya registrada", id: existing.id }), {
        status: 409,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
  ...
  if (insertError) {
    ...
    return new Response(JSON.stringify({ error: "Error interno al guardar el registro" }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
